...
namespace Rift {
    ...
    bool RiftHTTP::ParseURL(const std::string& url, std::string& host, std::string& path, short& port, bool& isSecure) {
        if (url.empty() || url.find("://") == std::string::npos) {
            std::cout << "[RiftNet]: Invalid URL provided.\n";
            return false;
        }
    ...
    bool RiftHTTP::ValidateResponseCert(HINTERNET hRequest) {
        ...
        std::cout << "[RiftNet]: Invalid SSL Certificate.\n";
        return false;
    }
    ...
    bool RiftHTTP::SendHTTPRequest(
        ...
        if (!hSession) {
            std::cout << "[RiftNet]: Failed to open internet session.\n";
        ...
        if (!ParseURL(url, host, path, port, isSecure)) {
            std::cout << "[RiftNet]: Invalid URL format.\n";
        ...
        if (!hConnect) {
            std::cout << "[RiftNet]: Failed to connect to server.\n";
            InternetCloseHandle(hSession);
            return false;
        }
        ...
        if (!hRequest) {
            std::cout << "[RiftNet]: Failed to open request.\n";
        ...
        if (!result) {
            std::cout << "[RiftNet]: Failed to send request.\n";
