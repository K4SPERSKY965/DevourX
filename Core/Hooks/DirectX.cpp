...
LRESULT __stdcall dWndProc(const HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	...
	if (KeyBinds::IsKeyPressed(settings.KeyBinds.Toggle_Menu))
	{
		...
		if (settings.bShowMenu && XUtility::IsInGame()) app::UIHelpers_ShowMouseCursor(NULL);
		if (not settings.bShowMenu && XUtility::IsInGame()) app::UIHelpers_HideMouseCursor(NULL);
	}
...
HRESULT __stdcall dPresent(IDXGISwapChain* __this, UINT SyncInterval, UINT Flags) {
	...
	if (!settings.ImGuiInitialized) {
		if (ImGuiInitialization(__this)) {
			...
			std::cout << "[DevourX]: ImGui Initialized successfully!\n";
			std::cout << "[DevourX]: Fullscreen: " << app::Screen_get_fullScreen(nullptr) << std::endl;
			std::cout << "[DevourX]: DirectX Window Size: " << +size.x << "x" << +size.y << std::endl;
		}
		else {
			ReleaseSemaphore(DirectX::hRenderSemaphore, 1, NULL);
